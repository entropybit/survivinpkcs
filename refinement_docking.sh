...
if false; then
    mkdir $FOLDER/monomer_refined
    ls $FOLDER/sur_monomer | grep _0001_ | grep .pdb | parallel $RS_EXEC/$DOCK -docking:partners C_A -docking_local_refine -nstruct 1 -database $RS_DATA -score:docking_interface_score 1 -native $FOLDER/pk_mono_sur_mono.pdb --overwrite -out:prefix $FOLDER/monomer_refined/ -out:file:scorefile monomer_scores.sc -s $FOLDER/sur_monomer/{} ::: ${FILES[@]}
fi
